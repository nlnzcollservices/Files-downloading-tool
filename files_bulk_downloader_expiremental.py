...
def download_file(url, save_directory, progress_text, file_count, csv_writer):
    ...
    filename = None
    if "youtube.com" in url or "youtu.be" in url or "rumble.com":
        ...
        if response.status_code in [200, 202, 203]:
            ...
            print("here16")
            if file_extension and file_extension.lower() in supported_extensions:
                print("here17")
                with open(save_path, 'wb') as fl:
                    fl.write(response.content)
                    file_count[0] += 1
                    csv_writer.writerow([url, response.url, filename, "Done"])
                    message =  f"Downloaded File {file_count[0]} of {file_count[1]}: {save_path} -  {url}\n"          
                    
                    progress_text.insert(END, message)

            elif not file_extension:
                message = f"Not a file: {url}, {response.url},{filename}\n"
                csv_writer.writerow([url,response.url, filename, "Not a file"])
                progress_text.insert(END, message)    

            else:
                message = f"Skipped File: {url}, {response.url},{filename}\n"
            ...
            message = f"Failed to download file from URL:{url}, {response.url}, {url}\n"
